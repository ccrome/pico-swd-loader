...
static bool start(uint pc, uint sp) {
    ...
    uint entry_point = 0x20000001; // _entry_point with thumb bit set
    
    mp_printf(&mp_plat_print, "Set PC to _entry_point at %08x\n", entry_point);
    if (!write_reg(0xe000edf8, entry_point) ||
    ...
    mp_printf(&mp_plat_print, "Resume at _entry_point\n");
    if (!write_reg(0xe000edf0, 0xA05F0001)) {
        mp_printf(&mp_plat_print, "Resume failed\n");
        return false;
    }
...
static bool swd_load_program_internal(const uint* addresses, const uint** data, const uint* data_len_in_bytes, uint num_sections, uint pc, uint sp, bool use_xip_as_ram) {
    ...
    // Don't start core 1 - it's not needed for simple blink
    // and may interfere with core 0
    mp_printf(&mp_plat_print, "Core 1 left idle (not needed for this firmware)\n");
